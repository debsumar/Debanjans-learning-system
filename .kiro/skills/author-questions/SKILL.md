...
Space-separate multiple objective ids. Untagged recall items silently break objective-level reporting and the study brief: `study.js` cannot attribute their attempts, misses, or high-confidence misses to an objective, so objective weakness and brief output lose those items. MCQ answer panels continue to require `data-objective` too. Verify that every recall item carries BOTH `data-recall` and `data-objective`, every tag names an existing registry objective, and each objective has intended recall coverage.
